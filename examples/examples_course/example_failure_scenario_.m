...
for time = p_sim.start_time:p_sim.dt:p_sim.end_time
    ...
    %%% Examples of individual UAVs with break-away behavior

    % UAV #2 suddenly flies towards the ground (will still evade obstacles)
    if time == 100
        swarm.drones(2).swarm_in_calcs = false;
        swarm.drones(2).swarm_control = 'velocity'; % Control it by setting a new velocity
        swarm.drones(2).v_ref = 12; % Speed
        swarm.drones(2).u_ref = [0; 0; 1]; % Direction (unit vector)
    end

    % UAV #5 suddenly begins flying back to the starting point
    if time == 150
        swarm.drones(5).swarm_in_calcs = false;
        swarm.drones(5).swarm_control = 'goal'; % Control by setting a new goal coordinate
        % No change to speed
        swarm.drones(5).x_goal = p_swarm.P0;
    end
