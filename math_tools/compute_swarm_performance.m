...
min_d_obs = inf(t_steps,nb_agents);

% Init flag if number of agents = 1
flag_one_agent = (nb_agents == 1);

%% Loop over time

for k = 1:t_steps
    
    % Some metrics should not run if number of agents = 1
    if ~flag_one_agent
    ...
    end % end - run metrics depending on number of agents
    
    %% Safety with obstacles: reflects the number of collisions between swarm agents and obstacles
    
    nb_possible_coll = 0;
    if p_swarm.is_active_spheres || p_swarm.is_active_cyl || ...
            p_swarm.is_active_arena || p_swarm.is_active_blocks
        
        pos_k = pos_history(k,:);
        pos_k = reshape(pos_k,3,[]); % coordinates (rows) of each drone (cols)
        
        if p_swarm.is_active_spheres
            c_spheres = p_swarm.spheres(1:3,:);
            r_spheres = p_swarm.spheres(4,:);
            
            % Distances from each agent to center of each sphere
            D_spheres = pdist2(pos_k',c_spheres');

            % Distance from each agent to each sphere
            temp_dist = min(pdist2(pos_k(1:3,:)',c_spheres') - repmat(r_spheres, nb_agents, 1),[],2); 
            temp_dist = temp_dist';
            temp_elem = temp_dist < min_d_obs(k,:);
            min_d_obs(k, temp_elem) = temp_dist(temp_elem);
            % min_d_obs(k,:) = min(pdist2(pos_k(1:3,:)',c_spheres') - repmat(r_spheres, nb_agents, 1),[],2); 
            
            % Count if collision
            nb_obs_coll(k) = nb_obs_coll(k) + sum(sum(D_spheres <= repmat(r_spheres+p_swarm.r_coll, nb_agents, 1)));
        ...
        if p_swarm.is_active_cyl
            ...
            % Distances from each agent to center of each cylinder
            D_cyl = pdist2(pos_k(1:2,:)',c_cyl');
            temp_dist = min(pdist2(pos_k(1:2,:)',c_cyl') - repmat(r_cyl, nb_agents, 1),[],2); 
            temp_dist = temp_dist';
            temp_elem = temp_dist < min_d_obs(k,:);
            min_d_obs(k,temp_elem) = temp_dist(temp_elem);

            % Count if collision
            nb_obs_coll(k) = nb_obs_coll(k) + sum(sum(D_cyl <= repmat(r_cyl+p_swarm.r_coll, nb_agents, 1)));

            % Count possible collision opportunities
            nb_possible_coll = nb_possible_coll + nb_agents*length(r_cyl);
        end
        if p_swarm.is_active_arena
            x_coll = sum(pos_k(1,:) <= (p_swarm.x_arena(1,1)+p_swarm.r_coll) | ...
                pos_k(1,:) >= (p_swarm.x_arena(1,2)-p_swarm.r_coll));
            y_coll = sum(pos_k(2,:) <= (p_swarm.x_arena(2,1)+p_swarm.r_coll) | ...
                pos_k(2,:) >= (p_swarm.x_arena(2,2)-p_swarm.r_coll));
            z_coll = sum(pos_k(3,:) <= (p_swarm.x_arena(3,1)+p_swarm.r_coll) | ...
                pos_k(3,:) >= (p_swarm.x_arena(3,2)-p_swarm.r_coll));
        ...
        if isfield(p_swarm, 'is_active_blocks') && p_swarm.is_active_blocks
            for i = 1:size(pos_k, 2)
                for j = 1:p_swarm.n_blocks
                    ...
                    % Check distance from agent to block
                    [~, dist_check, ~] = obstacles.check_inside_block(...
                        pos_k(:,i)', ...
                        p_swarm.blocks_limits(1:2, j), ...
                        p_swarm.blocks_limits(3:4, j), ...
                        p_swarm.blocks_limits(5:6, j));

                    % Save distance
                    if dist_check < min_d_obs(k,i)
                        min_d_obs(k,i) = dist_check;
                    end
                    % Count if collision
                    if dist_check <= p_swarm.r_coll
                        nb_obs_coll(k) = nb_obs_coll(k) + 1;
                    end
                end
            end
            % Count possible collision opportunities
            nb_possible_coll = nb_possible_coll + nb_agents*p_swarm.n_blocks;
        end
...
%% Print warning if number of agents == 1
if flag_one_agent
    warning('Swarm only has one agent; many of the swarm performance metrics are invalid.')
end
